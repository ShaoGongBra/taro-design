...
const TabMenu = ({ attr, hover, change, child }) => <View className={`form-tab__nav${['top', 'bottom'].includes(attr.position) ? ' form-tab__nav--row' : ''}`}>
...
const Tab = ({ children, onEvent, _childNodes, ...attr }) => {
  ...
  return <View className={classNames(attr.className, 'form-tab', { 'form-tab--row': row })} style={attr.style}>
    {tabStart && <TabMenu attr={attr} child={_childNodes} hover={hover} change={change} />}
