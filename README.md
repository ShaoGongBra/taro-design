...
```
对应的JSX代码如下，这些组件并不是原生的Taro组件，二十经过封装的，所以你看到下面的`Text`组件的文本并不是这样：`<Text>文本内容<Text>`，而是将文本内容赋值在其text属性上
```html
<View>
  <Text text='文本内容' />
</View>
...
在线模板现在未开放注册账号以及管理功能，请使用下面的账号密码进行体验，你保存的模板和项目也可能在后面的更新中被删除。  
用户名：admin  
密码：123456
